      ...
      <el-table-column label="操作" header-align="center" width="200">
        <template #default="scope">
          <div style="margin-left: 35%">
            ...
            <el-button size="small" type="danger" @click="handleEdit(scope.row,0)" v-if="scope.row.is_operate">封号</el-button>
         
            <el-button size="small" type="info" @click="handleEdit(scope.row,1)" v-else>解封</el-button>
         </div>
        </template>
      </el-table-column>
    </el-table>
...
// 获取所有学生的数据
const get_student_data = async (val) => {
  try {
    const res = await Get_Student_Data(val);
    let datas = [];
...
const handleCurrentChange = async(val) => {
  ...
  get_student_data({page:val,size:pagesize.value})
};
...
const SearchForm = async () => {
  try {
    const res = await Get_Search_Student_Data(Search_Data);
    if (res.status == 201) {
...
// 封号
const handleEdit  =(val,operate)=>{
  let title = ''
  operate==0?title='封号':title='解封'
